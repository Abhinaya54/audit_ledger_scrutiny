...
interface OverviewTabProps {
  ...
  risk: {
    high: { count: number; exposure: number };
    medium: { count: number; exposure: number };
    low: { count: number; exposure: number };
  };
  controls: ControlRow[];
  onUploadClick: () => void;
  onSwitchToInvestigation: () => void;
}
    ...
    <>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button
            onClick={onSwitchToInvestigation}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
        <KpiCard title="Total Transactions" value={formatNumber(summary.total_entries)} />
